...
const application = (state = INITIAL_STATE.application, action = {}) => {
  switch (action.type) {
    ...
    case ActionTypes.APPLICATION_MESSAGE_REFRESH: {
      ...
      if ((state.selectedFolderId === action.payload.folder.folderId) && (state.selectedMessage.uid === message.uid)) {
    ...
    case ActionTypes.APPLICATION_MESSAGE_REPLACE_IMAGE: {
      ...
      if (contentId.length > 0) {
        ...
        // Store in application.downloadedMessages
        newState.downloadedMessages[message.messageId].content = newState.downloadedMessages[message.messageId].content.replace(regex, objectUrl);

        // Update selected message if applicable
        if ((newState.selectedFolderId === folder.folderId) && (newState.selectedMessage.uid === message.uid)) {
